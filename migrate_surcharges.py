...
def main():
    ...
    print(f"Skipped (send invoice):         {len(results['skipped_send_invoice'])}")
    ...
    if results["skipped_send_invoice"]:
        print("\n-- Skipped (send invoice / not auto-charge) --")
        for r in results["skipped_send_invoice"]:
            print(f"  {r}")
